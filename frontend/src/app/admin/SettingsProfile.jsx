...
const SettingsProfile = () => {
  ...
  const { mutate: updatePassword, isPending: changingPwd } = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      const freshUser = result?.user || (await authService.me())
      if (freshUser) {
        queryClient.setQueryData(['admin-my-profile'], { user: freshUser, employee: freshUser })
      }
      queryClient.invalidateQueries({ queryKey: ['admin-my-profile'] })
      if (mustChangePassword) {
        toast.success('Password set successfully! Your account is now unlocked.')
        navigate(getDashboardPath(freshUser), { replace: true })
      } else {
    ...
    <AdminLayout title="Settings & Profile">
      <div className="min-h-full p-5 space-y-5">
        ...
        <div className="flex gap-1 border-b border-gray-200">
          {TABS.map(tab => {
            const Icon = tab.icon
            const isTabDisabled = mustChangePassword && tab.id !== 'security'
            return (
              <button
                key={tab.id}
                disabled={isTabDisabled}
                onClick={() => setActiveTab(tab.id)}
                title={isTabDisabled ? 'Set your new password first to unlock account' : undefined}
