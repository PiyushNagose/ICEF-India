...
const getExportContextRows = (context = {}) => [
  { label: "Export ID", value: () => context.exportId || "" },
  { label: "Exported By", value: () => context.requestedBy || "" },
  { label: "Exported At", value: () => formatDate(context.generatedAt || new Date()) },
];

const withExportContextHeaders = (headers, context = {}) =>
  context.exportId ? [...getExportContextRows(context), ...headers] : headers;
...
const buildReadme = ({ applications, generatedAt = new Date(), exportId, requestedBy }) => {
...
const buildPrintableRegisterHtml = (applications, context = {}) => {
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .trace { margin-top: 8px; font-size: 9px; color: #6b7280; }
  </style>
</head>
<body>
  <main class="sheet">
    <section class="header">
      ...
      <div class="trace">Export ID: ${htmlEscape(context.exportId || "-")} | Generated By: ${htmlEscape(context.requestedBy || "-")}</div>
    </section>
    <table>
      <thead>
        <tr>
...
const buildExportContent = (type, applications, context = {}) => {
  if (type === "documents") {
    return toCsv(flattenDocumentRows(applications), withExportContextHeaders(documentHeaders, context));
  }
  if (type === "payments") {
    return toCsv(applications, withExportContextHeaders(paymentHeaders, context));
  }
  if (type === "corrections") {
    return toCsv(applications, withExportContextHeaders(correctionHeaders, context));
  }
  if (type === "print") {
    return buildPrintableRegisterHtml(applications, context);
  }
  return toCsv(applications, withExportContextHeaders(applicationRegisterHeaders, context));
};

const buildGovernmentBundle = async (applications, context = {}) => {
  const generatedAt = context.generatedAt || new Date();
  const exportContext = { ...context, generatedAt };
  const zip = createZipBuffer([
    {
      name: "README.txt",
      content: buildReadme({ applications, ...exportContext }),
      date: generatedAt,
    },
    {
      name: "application-register.csv",
      content: buildExportContent("register", applications, exportContext),
      date: generatedAt,
    },
    {
      name: "payment-register.csv",
      content: buildExportContent("payments", applications, exportContext),
      date: generatedAt,
    },
    {
      name: "correction-register.csv",
      content: buildExportContent("corrections", applications, exportContext),
      date: generatedAt,
    },
    {
      name: "document-manifest.csv",
      content: buildExportContent("documents", applications, exportContext),
      date: generatedAt,
    },
    {
      name: "printable-application-register.html",
      content: buildPrintableRegisterHtml(applications, exportContext),
      date: generatedAt,
    },
  ]);

  return writeExportFile({
    filename: `government-handover-${context.exportId || Date.now()}.zip`,
    content: zip,
